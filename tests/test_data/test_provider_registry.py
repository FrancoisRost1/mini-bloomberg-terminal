...
from terminal.data.provider_alphavantage import AlphaVantageProvider
...
def test_production_selects_alpha_vantage(prod_config, monkeypatch):
...
def test_production_refuses_yfinance_even_if_configured(prod_config):
    # Simulate a misconfiguration where dev_provider somehow becomes primary.
    prod_config["data"]["primary_provider"] = "yfinance"
    registry = ProviderRegistry(prod_config)
    provider = registry.equity()
    # Production mode must refuse yfinance, returning None so the UI
    # surfaces DEGRADED instead of silently serving scraped data.
    assert provider is None


def test_production_without_alpha_vantage_key_is_none(prod_config, monkeypatch):
...
def test_env_var_overrides_config_mode(prod_config, monkeypatch):
    ...
    registry = ProviderRegistry(prod_config)
    # Env var wins: the registry should now be in development mode.
    provider = registry.equity()
    assert isinstance(provider, YFinanceProvider)


def test_polygon_stub_raises_on_use(prod_config, monkeypatch):
    monkeypatch.setenv("ALPHA_VANTAGE_API_KEY", "ignored")
