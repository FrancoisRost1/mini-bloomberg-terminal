...
def test_snapshot_returns_five_factors():
    ratios = {"pe_ratio": 15, "roe": 0.18, "market_cap": 5e9}
...
def test_momentum_positive_for_uptrend():
    ratios = {"pe_ratio": 15, "roe": 0.15, "market_cap": 1e10}
...
def test_value_inverse_of_pe():
    cheap = compute_factor_snapshot({"pe_ratio": 8, "roe": 0.1, "market_cap": 1e9}, _synth_prices())
    rich = compute_factor_snapshot({"pe_ratio": 40, "roe": 0.1, "market_cap": 1e9}, _synth_prices())
    assert cheap["factor_scores"]["value"] > rich["factor_scores"]["value"]


def test_quality_increases_with_roic():
    low = compute_factor_snapshot({"pe_ratio": 15, "roe": 0.05, "market_cap": 1e9}, _synth_prices())
    high = compute_factor_snapshot({"pe_ratio": 15, "roe": 0.35, "market_cap": 1e9}, _synth_prices())
...
def test_short_history_yields_nan_momentum():
    ...
    result = compute_factor_snapshot({"pe_ratio": 15, "roe": 0.1, "market_cap": 1e9}, short)
