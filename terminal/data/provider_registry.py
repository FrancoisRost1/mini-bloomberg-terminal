...
Production mode: Alpha Vantage only for equities. No silent yfinance
fallback. If Alpha Vantage is unavailable, upstream code must surface a
DEGRADED or DATA UNAVAILABLE state.

Development mode: yfinance is permitted and carries a visible DEV MODE flag.
"""
...
from .provider_alphavantage import AlphaVantageProvider
...
class ProviderRegistry:
    ...
    The registry holds lazily-constructed singletons. Healthcheck failures
    are propagated as the provider returning ``None`` from ``equity()``;
    the UI treats this as DEGRADED rather than crashing the process.
    """
    ...
    def _select_equity_name(self) -> str:
        if is_production(self.cfg):
            return str(self.cfg["data"].get("primary_provider", "alpha_vantage"))
    ...
    def _instantiate(self, name: str) -> MarketDataProvider | None:
        ...
        if name == "alpha_vantage":
            return AlphaVantageProvider(self.cfg)
        if name == "yfinance":
            if is_production(self.cfg):
                return None
