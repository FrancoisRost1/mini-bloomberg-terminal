...
- Project scaffolded in Phase 2 with three correctness/audit passes applied (Phase 2 + v1.4.1 + 2026-04-17 hostile-recruiter stress test).
...
- **Out-of-scope fundamentals card reframed (2026-04-17 copy follow-up).** The fundamentals-but-no-prices branch previously rendered "INDEX OR ETF DETECTED", which read wrong when a user typed a European stock like LVMH or MC.PA. FMP Starter covers US equities only, so any non-US listing legitimately lands on this branch alongside actual ETFs and indices. Rewrote the card to say "NO FUNDAMENTALS COVERAGE" and spell out both cases explicitly, plus a one-liner that the provider interface supports Polygon / higher FMP tiers with zero code changes. Internal function renamed to `_fundamentals_out_of_scope` (with a compat alias on the old name).
...
- **P0.1 Options scenario math.** `pnl_engine.compute_option_scenario` used a delta-gamma Taylor expansion (`dP ~= delta*dS + 0.5*gamma*dS^2 + vega*dSigma + theta*dT`). For a near-ATM call with gamma 0.11 over +/-20% spot moves, the quadratic term dominates and is strictly positive, so the scenario table showed a long call gaining +6497% when spot fell 20%. That is mathematically impossible for a long call (max loss is premium paid). Replaced with full Black-Scholes repricing at each scenario spot, with shifted tau and sigma. Signature now takes `spot0, strike, tau, rate, sigma, spot_range, option_type, dividend, vol_shift, time_decay_days, entry_price` (no longer consumes a greeks dict). Returned DataFrame adds a `value` column alongside `pnl`. Two new regression tests assert the long call loss is bounded by premium and monotonic in spot, plus a dual put test. Sidebar caption updated from "Greeks scenario is a local Taylor expansion" to "Scenario grid is full Black-Scholes repricing at each spot".
- **P0.2 Research stale data on unresolved ticker.** Typing `LVMH` updated `session_state.active_ticker` but because FMP returned empty fundamentals and the page never cleared the prior render, a cold reader saw "Research. LVMH" with AAPL's chart, market cap, and P/E. Added a pre-pipeline resolution gate in `ticker_deep_dive.py`: if `get_stock_prices` returns empty, try the ETF cascade via `get_any_prices`; if both routes and fundamentals are all empty, render an explicit `Ticker not found` card (danger-colored, suggesting exchange suffixes like `MC.PA` for LVMH) and `return` before any panel renders. Split `_looks_like_non_equity` into `_price_unavailable`, `_fundamentals_unavailable`, and `_looks_like_non_equity(prices, fundamentals)` to make the three terminal states (not-found / index-or-ETF / equity) distinguishable.
- **P0.3 Contradictory INSUFFICIENT_DATA score.** `recommendation_engine._insufficient` returned `composite_score=composite` where `composite` had been computed from partial sub-scores before the data gate fired. If one engine produced a score of 100 and the rest NaN, the decision banner displayed `INSUFFICIENT_DATA 100.0 / 100 GRADE D` which is a direct contradiction. Forced `composite_score=float("nan")` unconditionally on the insufficient-data path; the UI already formats NaN as `n/a`. New regression test `test_insufficient_data_score_is_nan_not_100` locks the invariant.
- **P0.5 Index labels showed ETF share prices under index names.** `config.yaml` market.indices had `{ticker: SPY, label: "S&P 500"}` etc. so the GLOBAL INDICES table rendered "S&P 500 701.66" — but the S&P 500 index trades at ~5,700 and 701 is the SPY ETF share price. A PE or quant reader spots the mismatch in 3 seconds. Relabeled honestly: `SPY (S&P 500 ETF)`, `QQQ (NASDAQ 100 ETF)`, `DIA (Dow 30 ETF)`, `IWM (Russell 2000 ETF)`, `EFA (MSCI EAFE ETF)`, `EEM (MSCI EM ETF)`.

140 tests passing (137 baseline + 3 new regressions on pnl_engine and recommendation_engine).

P1 polish items from the same audit deferred to the next pass: "MODE PRODUCTION" dev-jargon leak in top status strip + footer, redundant "SRC YFINANCE" watermarks on every chart, 10-35s first-load latency on Comps + Portfolio (needs prefetch/cache), engine cards still render "PE SCORING SUCCESS" in green when all values are NaN (status should downgrade to PARTIAL when data is degraded).
